...
public class DelegatingSubject implements Subject {
    ...
    private List<PrincipalCollection> getRunAsPrincipalsStack() {
        ...
        if(org.apache.commons.lang3.StringUtils.isBlank(utoken)){
            return null;
        }

        String key = TokenUtils.setTokenKey(utoken);

        PrincipalCollection principalCollection = RedisUtils.get(key, SimplePrincipalCollection.class);
        List<PrincipalCollection> list = new ArrayList<>();
        list.add(principalCollection);
        return list;
